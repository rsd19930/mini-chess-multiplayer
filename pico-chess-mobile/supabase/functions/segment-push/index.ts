  ...
  try {
    ...
    if (!title || !body || !player_ids || !Array.isArray(player_ids)) {
      return new Response(JSON.stringify({
        error: 'Missing title, body, or player_ids'
      }), {
        status: 400,
        headers: {
    ...
    let query = supabaseAdmin.from('players').select('expo_push_token').not('expo_push_token', 'is', null);
    ...
    if (player_ids.length === 1 && player_ids[0] === "ALL") {
      ...
      if (admin_secret !== Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')) {
        return new Response(JSON.stringify({
          error: 'Unauthorized: Invalid admin_secret for broadcast'
        }), {
          status: 401,
          headers: {
            'Content-Type': 'application/json',
    ...
    const messages = players.filter((p)=>p.expo_push_token && p.expo_push_token.startsWith('ExponentPushToken[')).map((p)=>({
