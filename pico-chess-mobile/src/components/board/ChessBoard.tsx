...
import { GameAction, GameState, Position, PieceType, PlayerColor } from '../../types';
...
export const ChessBoard: React.FC<ChessBoardProps> = ({ localColor, engine, gameState, setGameState, isInputDisabled = false }) => {
    ...
    const legalActions = useMemo(() => engine.getAllLegalActions(gameState), [gameState, engine]);
    ...
    const highlightedSquares = useMemo(() => {
        ...
        if (selectedPos) {
            legalActions.forEach(a => {
                if (a.type === 'move' && a.from.row === selectedPos.row && a.from.col === selectedPos.col) {
    ...
    const handleSquarePress = (row: number, col: number) => {
        ...
        const actionToApply = legalActions.find((a) => {
            if (a.type === 'move' && selectedPos) {
                return a.from.row === selectedPos.row && a.from.col === selectedPos.col && a.to.row === row && a.to.col === col;
            }
            if (a.type === 'drop' && selectedPocketPiece) {
                return a.pieceType === selectedPocketPiece && a.to.row === row && a.to.col === col;
            }
            return false;
        });
        ...
        if (actionToApply) {
            ...
            const isCapture = actionToApply.type === 'move' && gameState.board[actionToApply.to.row][actionToApply.to.col] !== null;
            ...
            if (newState.isGameOver) {
                if (newState.winReason === 'Checkmate') {
            ...
            } else if (actionToApply.type === 'drop') {
    ...
    const handlePocketSelect = (pieceColor: PlayerColor, pieceType: PieceType) => {
        if (isInputDisabled || gameState.turn !== pieceColor || gameState.pendingPromotion) return;
    ...
    const handlePromotionChoice = (pieceType: PieceType) => {
        ...
        try {
            ...
            if (newState.isGameOver) {
                ...
                AudioService.playMove(); // or a special promote sound if we had one
            }
        } catch (e) {
            console.error(e);
        }
    ...
    const renderSquares = () => {
        ...
        for (let r = 0; r < 6; r++) {
            for (let c = 0; c < 6; c++) {
                ...
                const isHighlighted = highlightedSquares.some(h => h.row === visualRow && h.col === visualCol);
                const isSelectedSquare = selectedPos?.row === visualRow && selectedPos?.col === visualCol;

                const isCapture = isHighlighted && piece && piece.color !== gameState.turn;
                const isThreatenedKing = gameState.inCheck && piece?.type === 'K' && piece?.color === gameState.turn;

                const finalBgColor = isThreatenedKing ? 'rgba(255, 0, 0, 0.5)' : bgColor;

                squares.push(
                    <TouchableWithoutFeedback key={`${visualRow}-${visualCol}`} onPress={() => handleSquarePress(visualRow, visualCol)}>
                        <View style={[
                            styles.square,
                            { backgroundColor: finalBgColor },
                            isSelectedSquare && styles.selectedSquare
                        ]}>
                            {isHighlighted && !isCapture && <View style={styles.highlightDot} />}
                            {isCapture && <View style={styles.captureRing} />}
                            {piece && (
                                <Animated.View style={styles.pieceAnimatedContainer}>
                                    <ChessPiece2D type={piece.type} color={piece.color} size={BOARD_SIZE / 6 * 0.8} />
                                </Animated.View>
                            )}
                        </View>
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.board25DWrapper}>
                ...
                <View style={styles.boardGridOverlay}>
                    {renderSquares()}
                </View>
                        ...
                        <View style={styles.promotionBox}>
                            ...
                            <View style={styles.promotionChoices}>
                                {(['R', 'B', 'N'] as PieceType[]).map(pt => (
                                    <TouchableWithoutFeedback key={pt} onPress={() => handlePromotionChoice(pt)}>
                                        <View style={styles.promotionOption}>
                                            <ChessPiece2D type={pt} color={gameState.turn} size={50} />
                                        </View>
                                    </TouchableWithoutFeedback>
                                ))}
                            </View>
