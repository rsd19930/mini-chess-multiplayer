...
export async function calculateBotAction(
  ...
  if (evaluatedActions.length > 0) {
    if (evaluatedActions.length >= 2) {
      let probabilityBest = 0.5; // Default Medium

      if (depth === 1) {
        probabilityBest = 0.3; // Easy: 30% Best, 70% 2nd Best
      } else if (depth === 2) {
        probabilityBest = 0.5; // Medium: 50% Best, 50% 2nd Best
      } else if (depth >= 3) {
        probabilityBest = 0.7; // Hard: 70% Best, 30% 2nd Best
      }
