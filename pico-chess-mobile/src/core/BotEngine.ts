...
export async function calculateBotAction(
    ...
    if (evaluatedActions.length > 0) {
        // Inject "Human Error" (Calculated Blunders)
        // Give the Bot an equal 33% chance to pick the 1st, 2nd, or 3rd best evaluated move
        const maxIndex = Math.min(3, evaluatedActions.length);
        const randomIndex = Math.floor(Math.random() * maxIndex);
        const bestAction = evaluatedActions[randomIndex].action;
