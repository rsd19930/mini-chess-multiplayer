...
export class GameEngine {
  ...
  private cloneState(state: GameState): GameState {
    return {
      ...
      pocket: {
        white: [...state.pocket.white],
        black: [...state.pocket.black],
      },
      pendingPromotion: state.pendingPromotion,
      last_move_timestamp: state.last_move_timestamp,
    };
  }
    ...
    for (let r = 0; r < 6; r++) {
      for (let c = 0; c < 6; c++) {
        ...
        if (piece && piece.color === color) {
          const pseudoMoves = this.getPiecePseudoLegalMoves(
            state,
            { row: r, col: c },
            piece,
          );
  ...
  private getPiecePseudoLegalMoves(
    state: GameState,
    from: Position,
    piece: Piece,
  ): Move[] {
        ...
        if (this.isWithinBounds(from.row + dir, from.col + dc)) {
          ...
          if (target && target.color === opponent) {
            this.addPossiblePromotions(
              moves,
              from,
              { row: from.row + dir, col: from.col + dc },
              piece,
            );
          }
        }
