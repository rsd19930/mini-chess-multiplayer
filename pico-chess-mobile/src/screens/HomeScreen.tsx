        ...
        const syncUserData = async (activeSession: any) => {
            ...
            try {
                const { data: playerData, error } = await supabase
                    .from("players")
                    .select("coins, last_login_bonus, rating")
                    .eq("id", userId)
                    .single();
                if (playerData && !error && mounted) {
                    setCoinBalance(playerData.coins);
                    setPlayerElo(playerData.rating ?? 1000);
    ...
    return (
        <View style={styles.container}>
            ...
            <ImageBackground
                ...
                resizeMode="cover"
            >
                {/* Header Section (Player Profile / Sign In) */}
                <View style={[styles.header, { paddingTop: Math.max(insets.top, 20) }]}>
                    {session ? (
                        ...
                        >
                            ...
                            {/* Bottom Row: Full-width Tier Bar */}
                            <View style={styles.tierBar}>
                                <Text style={styles.tierBarText}>
                                    {getTierForElo(playerElo).icon} {getTierForElo(playerElo).name} · {playerElo}
                                </Text>
                            </View>
                        </TouchableOpacity>
                    ) : (
                        ...
                        >
                            <Text style={styles.signInPillText}>Sign In with Google</Text>
                        </TouchableOpacity>
                    )}
